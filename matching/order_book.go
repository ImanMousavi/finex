...
type OrderBook struct {
	...
	marketPrice      *decimal.Big // current market price
...
func makeComparator(priceDescending bool) LessFunc {
	...
	return func(a, b OrderTracker) bool {
		...
		if a.Price == b.Price { // if prices are equal, compare timestamps
			return a.Timestamp < b.Timestamp
		}
		if a.Price < b.Price { // if a price is less than b return true if ascending, false if descending
			return sort
		}
...
func NewOrderBook(instrument string, marketPrice *decimal.Big, tradeBook *TradeBook, orderRepo OrderRepository) *OrderBook {
...
// Get a market price.
func (o *OrderBook) MarketPrice() *decimal.Big {
...
// Set a market price.
func (o *OrderBook) SetMarketPrice(price decimal.Big, fPrice float64) {
...
func (o *OrderBook) Add(order Order) (bool, error) {
	if order.Type == TypeMarket && !(order.Price.Sign() == 0) {
		return false, ErrInvalidMarketPrice
	}
	if order.Type == TypeLimit && order.Price.Sign() == 0 {
		return false, ErrInvalidLimitPrice
	}
	if order.Params.Is(ParamStop) && order.StopPrice.Sign() == 0 {
		return false, ErrInvalidStopPrice
	}

	orderPrice, ok := order.Price.Float64()
	if !ok {
		return false, errors.New("can't parse orderPrice")
	}
	...
	if order.Params.Is(ParamStop) {
		...
		orderStopPrice, ok := order.StopPrice.Float64()
		if !ok {
			return false, errors.New("can't parse orderPrice")
		}
...
func min(q1, q2 decimal.Big) decimal.Big {
	if q1.Cmp(&q2) <= 0 {
		return q1
	}
	return q2
...
func (o *OrderBook) matchOrder(orderPrice float64, order *Order, offers *orderMap) (bool, error) {
	...
	for iter := offers.Iterator(); iter.Valid(); iter.Next() {
		...
		if orderUnfilledQty := order.UnfilledQty(); currentAON && qty.Cmp(&orderUnfilledQty) != 0 {
			continue // couldn't find a match - we require AON but couldn't fill the order in one trade
		}
		if oppositeOrderUnfilledQty := order.UnfilledQty(); oppositeAON && qty.Cmp(&oppositeOrderUnfilledQty) != 0 {
		...
		BaseContext.Add(&order.FilledQty, &order.FilledQty, &qty)
		BaseContext.Add(&oppositeOrder.FilledQty, &oppositeOrder.FilledQty, &qty)

		matched = true
		oppositeOrderUnfilledQty := oppositeOrder.UnfilledQty()
		oppositeOrderFilledQty := oppositeOrder.FilledQty
		if oppositeOrderUnfilledQty.Sign() == 0 { // if the other order is filled completely - remove it from the order book
		...
		var Neg_oppositeOrderFilledQty decimal.Big
		Neg_oppositeOrderFilledQty.Neg(&oppositeOrderFilledQty)

		o.depth.UpdatePriceLevel(
			oppositeOrder.Side,
			oppositeOrder.Price,
			Neg_oppositeOrderFilledQty,
			!oppositeOrder.IsFilled(),
		) // Sub amount from pricelevel

		var MakerOrder Order
		var TakerOrder Order

		if order.ID < oppositeOrder.ID {
			MakerOrder = *order
			TakerOrder = oppositeOrder
		} else {
			MakerOrder = oppositeOrder
			TakerOrder = *order
		}
		var total decimal.Big
		BaseContext.Mul(&total, &price, &qty)

		trade := Trade{
			Instrument:   o.Instrument,
			MakerOrderID: MakerOrder.ID,
			TakerOrderID: TakerOrder.ID,
			MakerID:      MakerOrder.CustomerID,
			TakerID:      TakerOrder.CustomerID,
			Price:        price,
			Qty:          qty,
			Total:        total,
			Timestamp:    time.Now(),
	...
	// if order is not filled at all
	// this order will be add to pricelevel and depth
	if order.Type == TypeLimit {
		o.depth.UpdatePriceLevel(order.Side, order.Price, order.UnfilledQty(), true)
	}
...
func panicOnOrderType(order Order) {
	panic(fmt.Errorf("order type \"%v\" not implemented", order.Type))
}
